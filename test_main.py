...
LR_A = tf.compat.v1.train.exponential_decay(LR_A, GLOBAL_STEP, 10000, .97, staircase=True)
LR_C = tf.compat.v1.train.exponential_decay(LR_C, GLOBAL_STEP, 10000, .97, staircase=True)
...
class Actor(object):
    ...
    def _build_net(self, s, scope, trainable):
        with tf.compat.v1.variable_scope(scope):
            init_w = tf.compat.v1.random_normal_initializer(0., 0.01)
            init_b = tf.compat.v1.constant_initializer(0.01)
            net = tf.compat.v1.layers.dense(s, 500, activation=tf.nn.relu,
                                  kernel_initializer=init_w, bias_initializer=init_b, name='l1', trainable=trainable)
            net = tf.compat.v1.layers.dense(net, 200, activation=tf.nn.relu,
...
class Critic(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate, gamma, t_replace_iter, a, a_):
        ...
        with tf.compat.v1.variable_scope('abs_TD'):
        ...
        with tf.compat.v1.variable_scope('TD_error'):
            self.loss = tf.reduce_mean(input_tensor=self.ISWeights * tf.math.squared_difference(self.target_q, self.q))

        with tf.compat.v1.variable_scope('C_train'):
            self.train_op = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(self.loss, global_step=GLOBAL_STEP)
    ...
    def _build_net(self, s, a, scope, trainable):
        with tf.compat.v1.variable_scope(scope):
            ...
            with tf.compat.v1.variable_scope('l2'):
                net = tf.compat.v1.layers.dense(net, 20, activation=tf.nn.relu, kernel_initializer=init_w,
