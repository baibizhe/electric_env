...
class FooEnv(gym.Env):
    ...
    def __init__(self,max_buyer_price=300,max_seller_price=400,min_seller_price=300,min_buyer_price=450,\
                 num_of_seller =4,num_of_buyer = 3):
        ...
        self.action_space = spaces.Box(low=np.array([-self.deltaprice for i in range(num_of_seller)]), high=np.array([self.deltaprice for i in range(num_of_seller)]),
                                           shape=(num_of_seller,))
        self.observation_space = spaces.Box(low=np.array([0 for i in range(num_of_seller)]),high=np.array([self.max_seller_price for i in range(num_of_seller)]),shape=(num_of_seller,))
        # self.reset()
    ...
    def step(self, action):
        ...
        seller_num = [[self.state[i],self.seller_volum[i]] for i in range(self.num_of_seller)] #构建卖方的价格和容量
